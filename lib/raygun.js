...
var Raygun = function () {
  var _apiKey, _filters, raygun = this, _user, _version, _host, _port, _useSSL, _onBeforeSend, _offlineStorage, _isOffline, _cachePath;
  ...
  raygun.init = function (options) {
    ...
    if(_isOffline) {
      if(!_cachePath) {
        throw new Error("Cache Path must be defined before Raygun can cache offline");
      }

      _offlineStorage.init(_cachePath);
    }
  ...
  raygun.send = function (exception, customData, callback, request, tags) {
    ...
    if(_isOffline) {
      _offlineStorage.save(transportMessage);
    } else {
